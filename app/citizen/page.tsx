...
export default function CitizenPage() {
  ...
  React.useEffect(() => {
    const timer = setTimeout(() => {
      const onboardCompleted = localStorage.getItem('civichero_onboard_completed') === 'true';
      if (!onboardCompleted) {
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      case 'home':
        return (
          <ErrorBoundary title="Citizen Feed Error" description="The neighborhood feed was unable to load correctly. Please verify your connection or reset the demo data.">
            <CitizenFeed 
      ...
      case 'map':
        return (
          <ErrorBoundary title="Safety Map Error" description="The interactive GIS map suffered a loading anomaly. Try resetting the component.">
            <motion.div 
      ...
      case 'safety':
        ...
        return (
          <ErrorBoundary title="Safety Center Error" description="The neighborhood safety desk was unable to synchronize active hazard telemetry.">
            <motion.div 
              initial={{ opacity: 0, y: 16 }}
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: -16 }}
      ...
      case 'alerts':
        const resolvedIssues = [...issues]
          ...
          .sort((a, b) => getSortScore(b) - getSortScore(a));
        return (
          <ErrorBoundary title="Bulletins Error" description="The regional announcements ledger was unable to coordinate with live nodes.">
            <motion.div 
              initial={{ opacity: 0, y: 16 }}
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: -16 }}
      ...
      case 'search':
        return (
          <ErrorBoundary title="Search Desk Error" description="The decentralized global query indexing nodes had a synchronization anomaly.">
            <GlobalSearch />
          </ErrorBoundary>
        );
      case 'notifications':
        return (
          <ErrorBoundary title="Alerts Desk Error" description="Real-time alert replication is temporarily undergoing maintenance.">
            <NotificationCenter />
          </ErrorBoundary>
        );
      case 'passport':
        return (
          <ErrorBoundary title="Passport Desk Error" description="The Civic Passport ledger could not verify your identity.">
            <CivicPassport />
          </ErrorBoundary>
        );
      case 'settings':
        return (
          <ErrorBoundary title="Settings Panel Error" description="Your local configuration values are temporarily unavailable.">
            <SettingsPanel />
          </ErrorBoundary>
        );
      default:
